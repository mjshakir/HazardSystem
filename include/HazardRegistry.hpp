...
namespace HazardSystem {
    ...
    class HazardRegistry {
        ...
        public:
            ...
            //--------------------------
            explicit HazardRegistry(const size_t& capacity = 0)
                : m_capacity(capacity_size(capacity)), m_mask(m_capacity - 1U),
        ...
        protected:
            ...
            bool add_local(T* ptr) {
                ...
                for(size_t i = 0; i < m_capacity; ++i) {
                    ...
                    if(!_current or _current == _tomb) {
                        ...
                        if(m_slots[_idx].compare_exchange_weak(_expected, ptr,
            ...
            bool remove_local(T* ptr) {
                ...
                for(size_t i = 0; i < m_capacity; ++i) {
                    ...
                    if(_current == ptr) {
                        ...
                        uint32_t _count = m_counts[_idx].load(std::memory_order_acquire);
                        while(_count > 0 && !m_counts[_idx].compare_exchange_weak(
                                                _count, _count - 1, std::memory_order_acq_rel,
                                                std::memory_order_acquire)) {
                            /* retry */;
                        }
                        if(_count == 0) {
                        ...
                        (void)m_slots[_idx].compare_exchange_strong(
                            _expected, const_cast<T*>(_tomb), std::memory_order_acq_rel,
                            std::memory_order_acquire);
                        return true;
	                }// end if (_current == ptr)
                    //--------------------------
            ...
            std::vector<T*> snapshot_local(void) const {
                //--------------------------
                std::vector<T*> _hazards;
                _hazards.reserve(m_capacity / 2);
                //--------------------------
                const T* _tomb = tombstone();
                //--------------------------
                for(size_t i = 0; i < m_capacity; ++i) {
            ...
            size_t hash(const T* ptr) const {
                const auto _c_raw = static_cast<uintptr_t>(reinterpret_cast<uintptr_t>(ptr));
                return mix_hash(_c_raw) & m_mask;
