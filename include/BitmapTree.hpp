...
namespace HazardSystem {
    ...
    class HAZARDSYSTEM_API BitmapTree {
            //--------------------------------------------------------------
        private:
            static constexpr size_t C_WORD_BITS =
                static_cast<size_t>(std::numeric_limits<uint64_t>::digits);
            static constexpr size_t C_LEVEL_SHIFT = 6UL; // log2(64)
            //--------------------------
            static_assert((1ULL << C_LEVEL_SHIFT) == C_WORD_BITS,
                          "BitmapTree assumes 64-bit words");
            //--------------------------
            static constexpr size_t C_MAX_PLANES = 2UL;
            static constexpr size_t C_MAX_LEVELS =
                (C_WORD_BITS + (C_LEVEL_SHIFT - 1)) / C_LEVEL_SHIFT;
            //--------------------------
        ...
        public:
            ...
            std::optional<size_t> find_next(const size_t& start,
                                            const size_t& plane = 0) const noexcept;
            //--------------------------
        ...
        protected:
            ...
            //--------------------------
            std::optional<size_t> find_next_data(const size_t& start,
            ...
            //--------------------------
            std::atomic<uint64_t>&       word_data(const size_t& plane, const size_t& level,
                                                   const size_t& word_index) noexcept;
            const std::atomic<uint64_t>& word_data(const size_t& plane, const size_t& level,
                                                   const size_t& word_index) const noexcept;
            //--------------------------
            bool set_bit(const size_t& plane, const size_t& level,
                         const size_t& bit_index) noexcept;
            //--------------------------
            bool clear_bit(const size_t& plane, const size_t& level,
                           const size_t& bit_index) noexcept;
            //--------------------------
            std::optional<size_t> find_next_set_bit(const size_t& plane, const size_t& level,
                                                    const size_t& start_bit) const noexcept;
            //--------------------------
            std::optional<size_t> find_from_leaf(const size_t& plane,
